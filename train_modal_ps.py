"""Modal entrypoint for PS-only experiment (separate app to avoid conflicts)."""
...
image = (
    ...
    .uv_pip_install(
        "torch>=2.4", "torchvision>=0.19", "lightning>=2.4",
        "datasets", "numpy", "wandb", "omegaconf", "pyyaml",
...
app = modal.App("cosmorford-ps", image=image)
...
def train(config_path: str, experiment_name: Optional[str] = None):
    ...
    overlay = {"trainer": {"default_root_dir": str(checkpoint_dir), "callbacks": [
        {"class_path": "LearningRateMonitor", "init_args": {"logging_interval": "step"}},
        {"class_path": "ModelCheckpoint", "init_args": {"dirpath": str(checkpoint_dir), "monitor": "val_loss", "mode": "min", "save_top_k": 3, "save_last": True}},
    ]}}
    overlay_path = Path("/tmp/runtime_config.yaml")
    overlay_path.write_text(yaml.dump(overlay))
    cmd = ["trainer", "fit", f"--config=/root/{config_path}", f"--config={overlay_path}"]
    if last_ckpt.exists():
...
def main(config: str = "configs/default.yaml", name: Optional[str] = None):
